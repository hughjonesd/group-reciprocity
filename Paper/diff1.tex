...
\DIFaddbegin \citep{Pietraszewski2016470}\DIFaddend .

%DIF >  1. Why would group-based upstream reciprocity be important for the evolution of human behavior?  
%DIF >  How does this contribute
%DIF >  to current debates in the literature about the evolution of human behavior?
%DIF >  here's my first shot, is it too speculative? Could we back it up?
%DIF >  in particular, do existing models explain how IG conflict arises? I have the impression that in some models it just 
%DIF >  "turns up" exogenously.
\DIFaddbegin 

\DIFadd{Intergroup reciprocity could be important for human evolution. First, it may structure intergroup conflicts, just as
...
\DIFadd{Second, group reciprocity could provide an evolutionary basis for outgroup stereotyping. }\DIFaddend Upstream group reciprocity
...
\DIFdelbegin \DIFdel{Upstream group reciprocity could
...
\DIFaddbegin \DIFadd{Our expectations were as follows. First, in Direct Reciprocity rounds, individuals' allocations to their TG partner
...
We also measured participants' social value orientation }\citep{van1999pursuit}\DIFadd{. It is plausible that willingness to group-reciprocate should
...
negative reciprocity in general may be linked to spite }\citep{johnstone2004evolution}\DIFadd{. So we test a non-directional hypothesis here.
}
...
in that specific outgroup. \DIFaddbegin \DIFadd{This supports the argument of }\cite{Pietraszewski2016470} \DIFadd{that 
group identity can modify the cost-benefit calculus of individuals deciding whether to 
extend a conflict. }\DIFaddend Unlike parochial altruism and within-group reciprocity, 
\DIFdelbegin \DIFdel{this phenomenon }\DIFdelend \DIFaddbegin \DIFadd{group reciprocity }\DIFaddend requires humans to differentiate between outgroups, possibly 
providing a cognitive basis for \DIFdelbegin \DIFdel{discrimination and prejudicetargeted against specific minorities}\DIFdelend \DIFaddbegin \DIFadd{intergroup stereotyping and prejudice}\DIFaddend .
...
%DIF >  deals with point 5
\DIFaddbegin \DIFadd{Since our study was conducted with students from a rich industrialized democracy, results may not
generalize to all cultures }\citep{henrich2010most}\DIFadd{. In particular, the link between intentions
and moral judgment may vary across cultures }\citep{barrett2016small}\DIFadd{, and this could affect how group 
reciprocity plays out in different societies.
}

\DIFaddend Upstream reciprocity is notoriously difficult to understand in evolutionary
...
\newpage
\section*{\DIFadd{Appendix B: Allocations in the DR condition}}

\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{figure}
\includegraphics[width=\maxwidth]{figure/plots_dr-1} \caption[Allocations in the Direct Reciprocity condition versus partner's kindness in the TG]{\DIFaddFL{Allocations in the Direct Reciprocity condition versus partner's kindness in the TG. Circles show individual data points (circle size proportional to number of observations). Lines show linear regressions.}}\label{fig:plots_dr}
\end{figure}
