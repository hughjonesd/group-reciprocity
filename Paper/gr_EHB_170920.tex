...
%There are three kinds of theories of the evolution of intergroup violence.
...
%In the appendix we run simulations showing how this can happen. 
...
%Group reciprocity provides an evolutionary basis for outgroup stereotyping Upstream group reciprocity has different cognitive requirements from related phenomena. 
...
and were helped 4 times, then they help with probability 3/5 = 60\percent.

After each generation, each individual changed type with independently drawn probability $\pi = 0.05$, copying a 
new type from one other individual in the population. An individual's probability of being copied was proportional 
to his or her own payoff. This could represent either a process of success-biased cultural transmission, or evolution
with full mixing across populations. The simulation ran until one strategy evolved to fixation in the whole population.

Our base simulation has $T = m = 60$ and $G = 10$ groups. We vary the benefit of being helped $b$, and the group size
$n$, from 12 to 30. In the ``short memory" variant we set $T = m = 30$. In the ``many groups" variant we set $G = 20$.
We ran ten simulations of each parameter configuration. Figure \ref{fig:simulations} shows the proportion of times
that GR evolved to fixation.
