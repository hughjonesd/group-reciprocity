...
environmental parameters. Individuals are divided into groups of size $n$. In each period, each individual has an opportunity to help or
harm a randomly chosen outgroup member (the ``target"). Helping has a fitness cost~$c$ while providing a benefit~$b$ to 
the target (alternatively, $c$ could be the actor's benefit from exploiting the target, thereby
...
reciprocators learn to cooperate with each other, while not helping
