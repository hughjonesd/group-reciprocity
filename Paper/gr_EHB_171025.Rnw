...
<<sims, warning = FALSE, error = FALSE>>=
...
predictor of whether GR would evolve to fixation: the mean correlation was \Sexpr{round(crf_test$estimate[2], 2)} when GR fixed and \Sexpr{round(crf_test$estimate[1], 2)} when selfishness fixed (p \Sexpr{cut_pval(crf_test$p.value)}).
