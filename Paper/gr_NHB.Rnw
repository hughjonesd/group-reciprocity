...
if (file.exists("E:/Dropbox/Dave_Ro'i/group and indiv reciprocity")) {
...
\usepackage[backend=biber,natbib=true,sorting=nyt,sortcites=true,maxnames=10,firstinits=false,uniquename=false]{biblatex} % Numbered citations
...
%David Hugh-Jones\thanks{School of Economics, University of East Anglia. E-mail: d.hugh-jones@uea.ac.uk.} \and Itay Ron\thanks{E-mail: itayron@gmail.com} \and Ro'i Zultan\thanks{Corresponding author. Department of Economics, Ben-Gurion University of the Negev. E-mail: zultan@bgu.ac.il.}}
...
Sometimes humans take revenge, not on the person who harmed them, but on other people from that person's group. This can lead to cycles of intergroup conflict and violence. We designed a laboratory experiment to test the hypothesis that humans practice group-based reciprocity: if someone harms or helps them, they harm or help other members of that person's group. We find that artificial and arbitrary groups trigger such group reciprocity when the original perpetrator's act was deliberate and unequivocal. In contrast, acts that are harmful, but are not clear norm violations, are reciprocated towards the actor, but have no effect on her group peers. Our results can help us understand the psychology and evolutionary mechanisms behind intergroup revenge.
...
reciprocate towards groups. Although field observations from conflict are highly
suggestive, they are loaded with individual and group context and history. Observing group reciprocity under
controlled laboratory conditions with artifical groups identifies group reciprocity as an innate human tendency.
Cleanly identifying group reciprocity requires controlling for three
confounds: individual level reciprocity, e.g. if subjects' actions affect an
entire group including the original actor who helped or harmed them; generalized reciprocity,
where subjects reciprocate not specifically towards the original actor’s group,
but towards other people in general; and strategic interactions, where apparent reciprocity
is driven by reputation-building. While previous work has not satisfied all three conditions simultaneously
\citep{gaertner2008whenrejection,stenstrom2008theroles,hugh-jones2013intergroup}, our experiment 
fulfils all three: subjects
can differentiate the original actor from his or her group members, they interact
both with these group members and with members of other groups, and we minimize strategic concerns by
not giving feedback about subjects' actions.

 After an initial group-formation stage, participants interacted in two
 strategic stages. The upstream action, in which the individual could be helped 
 or harmed by another person, was represented by a Trust Game (TG) \citep{berg1995trust}. 
 In this game, the Sender~(S) receives~150
 money-equivalent tokens, and chooses how many of them to send to the
 Responder~(R). The amount sent is multiplied by a factor of~3, so that R
 receives between 0~and~450 tokens, of which he can send any number back to S.
 The TG enables us to model two types of interactions. Whereas R is clearly kind
 when returning money (and nasty when exploiting a generous proposer by keeping
 the received amount), S's intentions are equivocal. Sending money can be driven
 by selfish expectations of reciprocity, while not sending can be driven by
 caution. Thus, while all subjects experience helpful or harmful actions, only senders
 experience actions that clearly reflect their counterpart's preferences and intentions \citep{gunnthorsdottir2002using,kimbrough2015norms}.
...
regs <- dfr_targets %>% group_by(roletrust) %>% do(
        give_mod  = lm(give ~ 0 + indcondition, .),
        disc_mod  = lm(discrimination ~ 0 + indcondition, .),
        recip_mod = lm(give ~ indcondition + indcondition:kindness, .),
        give_mod2 = lm(give ~ indcondition, .),
        disc_mod2 = lm(discrimination ~ indcondition, .)
      ) 

mycoeftest <- function (m) coeftest(m, cluster.vcov(m, ~ session))
mean_coefs <- regs %>% do(tidy(mycoeftest(.$give_mod))) 
disc_coefs <- regs %>% do(tidy(mycoeftest(.$disc_mod))) 
suppressWarnings({
  recip_coefs_orig <- regs %>% do(tidy(mycoeftest(.$recip_mod))) 
  recip_coefs <- recip_coefs_orig %>% filter(grepl('kindness', term))
  mean_sigs   <- regs %>% do(tidy(mycoeftest(.$give_mod2)))
})
recip_coefs$estimate[c(1,5)] <- 0 # to avoid formatting probs later
disc_sigs <- regs %>% do(tidy(mycoeftest(.$disc_mod2)))
...
latex(tbl[, -1], file = '', label = 'tab:results', rgroup = c('Senders', 'Responders'),
      ...
      insert.bottom = '\\justify Mean allocation (out of 70), mean discrimination, and reciprocity (marginal effect of TG partner\'s kindness on allocation) by condition. Robust standard errors clustered on sessions. Significance of comparison to Baseline is marked.$^{*}$, $^{**}$, and $^{***}$ indicate p $<$ 0.05, p $<$ 0.01, and p $<$ 0.001, respectively.')
...
baseline~35 ($p>\Sexpr{myfmt(min(mean_sigs$p.value[c(2:3,6:7)]))}$ for all comparisons). Nonetheless, as the second 
...
<<plots, fig.cap='Allocations in the Group Reciprocity condition versus partner\'s kindness in the TG. Circles show individual data points (circle size proportional to number of observations). Lines show linear regressions.', warning = FALSE>>=
...
with(dfr %>% filter(greciprocity == 1, roletrust=="Sender"), 
      sizeplot(kindness, give, pch = 19, col = alpha("darkgreen", .85), xlab = "Responder kindness", ylab = "Allocation", 
      main = "Senders")) 
...
with(dfr %>% filter(greciprocity == 1, roletrust=="Responder"), 
      sizeplot(kindness, give, pch = 19, col = alpha("navy", .65), xlab = "Sender kindness", ylab = "Allocation", 
      main = "Responders"))
...
\citep*{murphy2011measuring} stage and a collectivism scale measurement
...
The average payment was approximately \$18 for a duration of 70~minutes.
...
%D. H. J., I. R, and R. Z. designed the experiment. I. R. and R. Z. conducted the experiments. D. H. J. and R. Z. analysed the data and wrote the article. All authors approved the final version.
