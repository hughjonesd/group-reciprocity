...
do_report <- function (pop, gen, stats, shiny_plot) {
  message(gen)
  shiny_plot$generation <- c(shiny_plot$generation, gen)
...
simulate <- function (
  ...
  while (TRUE) {
    ...
    if (is.numeric(report) && (gen %% generations) %% report == 0) shiny_plotter(pop, gen %% generations, stats[gen,])
